...
def generate_figure():
    ...
    ax.plot(x, omega3, color='#1f77b4', linestyle='-', lw=2.0, label=r'Schoenberg isotropic kernel $\Omega_3(x) = \frac{\sin x}{x}$')
    ax.plot(x, kernel_11, color='#d62728', linestyle='--', lw=2.0, label=r'Longitudinal kernel $K_{11}(x) = 3\left(\frac{\sin x - x\cos x}{x^3}\right)$')
    ax.axhline(0, color='black', lw=0.8, ls=':')
    ax.fill_between(x, omega3, 0, where=(omega3 < 0), color='#1f77b4', alpha=0.15)
    ax.fill_between(x, kernel_11, 0, where=(kernel_11 < 0), color='#d62728', alpha=0.15)
